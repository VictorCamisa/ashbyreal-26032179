...
export default function InstitucionalHome() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
          <Link to="/" className="flex items-center gap-3">
            <img src={logoTaubateChopp} alt="Taubaté Chopp" className="h-9 w-9 rounded-full object-cover ring-2 ring-primary/30" />
          ...
          <div className="hidden md:flex items-center gap-6 text-sm text-muted-foreground">
            {['produtos', 'calculadora', 'sobre', 'faq', 'contato'].map((s) => (
              <a key={s} href={`#${s}`} className="hover:text-foreground transition-colors capitalize">{s}</a>
            ))}
          </div>
          <div className="flex items-center gap-2">
            <Link to="/ecommerce">
              <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground">
                <ShoppingCart className="w-4 h-4 mr-1.5" /> Loja
              </Button>
            </Link>
            <Link to="/auth">
              <Button size="sm" className="bg-primary text-primary-foreground font-semibold rounded-full px-5 shadow-md shadow-primary/20">
                Sistema
              </Button>
            </Link>
          </div>
      ...
      <section className="relative min-h-[90vh] flex items-center pt-16">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-24 sm:py-32">
          <motion.div initial="hidden" animate="visible" variants={stagger} className="max-w-3xl">
            <motion.p variants={fadeIn} className="text-primary font-semibold text-sm tracking-widest uppercase mb-4">
              Distribuidor Oficial Ashby
            </motion.p>
            <motion.h1 variants={fadeIn} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold leading-[1.05] tracking-tight mb-6">
              Chopp premium{' '}
              <span className="text-primary">para seu evento</span>
            </motion.h1>
            <motion.p variants={fadeIn} className="text-lg sm:text-xl text-muted-foreground max-w-xl mb-8 leading-relaxed">
            ...
            <motion.div variants={fadeIn} className="flex flex-col sm:flex-row gap-3">
              <Button
                size="lg"
                className="bg-primary text-primary-foreground font-bold px-8 h-13 rounded-full shadow-lg shadow-primary/25 hover:shadow-primary/40 transition-all"
              ...
              <Button
                size="lg"
                variant="outline"
                className="rounded-full px-8 h-13 border-border"
          ...
          >
            {stats.map((s) => (
              <motion.div key={s.label} variants={fadeIn} className="text-center sm:text-left">
      ...
      <section id="produtos" className="py-20 scroll-mt-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="mb-12">
            <motion.p variants={fadeIn} className="text-primary font-semibold text-sm tracking-widest uppercase mb-2">Nossos Produtos</motion.p>
          ...
          >
            {products.map((p) => (
              <motion.div key={p.name} variants={fadeIn}
                className="group rounded-2xl border border-border/50 bg-card overflow-hidden hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300"
              >
      ...
      {/* ── Calculadora ── */}
      <section id="calculadora" className="py-20 scroll-mt-20 bg-muted/30 border-y border-border/30">
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="mb-10">
            <motion.p variants={fadeIn} className="text-primary font-semibold text-sm tracking-widest uppercase mb-2">Ferramenta Gratuita</motion.p>
            <motion.h2 variants={fadeIn} className="text-3xl sm:text-4xl font-extrabold">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
              className="space-y-5"
            >
              <p className="text-primary font-semibold text-sm tracking-widest uppercase">Sobre Nós</p>
            ...
            >
              <div className="w-64 h-64 sm:w-72 sm:h-72 rounded-3xl bg-primary/5 border border-primary/10 flex items-center justify-center">
                <img src={logoTaubateChopp} alt="Taubaté Chopp" className="w-44 h-44 rounded-full object-cover shadow-xl ring-4 ring-primary/20" />
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-20 bg-muted/30 border-y border-border/30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="mb-12">
            <motion.p variants={fadeIn} className="text-primary font-semibold text-sm tracking-widest uppercase mb-2">Depoimentos</motion.p>
            <motion.h2 variants={fadeIn} className="text-3xl sm:text-4xl font-extrabold">
      ...
      <section id="faq" className="py-20 scroll-mt-20">
        <div className="max-w-2xl mx-auto px-4 sm:px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="mb-10">
            <motion.p variants={fadeIn} className="text-primary font-semibold text-sm tracking-widest uppercase mb-2">FAQ</motion.p>
            <motion.h2 variants={fadeIn} className="text-3xl sm:text-4xl font-extrabold">
          ...
          <Accordion type="single" collapsible className="space-y-3">
            {faqItems.map((item, i) => (
              <AccordionItem key={i} value={`item-${i}`}
                className="border border-border/50 rounded-xl px-5 bg-card data-[state=open]:border-primary/30 transition-colors"
              >
                <AccordionTrigger className="text-left hover:no-underline py-4 text-sm font-medium hover:text-primary transition-colors">
                  {item.q}
                </AccordionTrigger>
      ...
      {/* ── Contato ── */}
      <section id="contato" className="py-20 scroll-mt-20 bg-muted/30 border-t border-border/30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            >
              <div className="space-y-4">
                ...
                <Button
                  className="w-full h-12 bg-primary text-primary-foreground font-bold rounded-lg shadow-md shadow-primary/20 hover:shadow-primary/30 transition-all"
      ...
      <a href={`https://wa.me/${WHATSAPP_NUMBER}`} target="_blank" rel="noopener noreferrer"
        className="fixed bottom-5 right-5 z-50 w-14 h-14 bg-green-500 hover:bg-green-600 rounded-full shadow-lg shadow-green-500/30 flex items-center justify-center transition-all hover:scale-110"
      >
